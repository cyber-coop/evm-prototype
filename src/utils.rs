...
pub fn save_state(db: &InMemoryDB, block_heigh: u64) {
    ...
    let mut states_file = std::fs::OpenOptions::new()
        .write(true)
        .truncate(true)
        .create(true)
        .open("./data/states.json")
        .unwrap();
    states_file.write_all(states.as_bytes()).unwrap();
    states_file.flush().unwrap();

    let mut height_file = std::fs::OpenOptions::new()
        .write(true)
        .truncate(true)
        .create(true)
        .open("./data/height.txt")
        .unwrap();
    height_file
        .write_all(block_heigh.to_string().as_bytes())
        .unwrap();
...
pub fn calculate_state_root(accounts: impl IntoIterator<Item = (Address, DbAccount)>) -> Vec<u8> {
    ...
    for account in accounts.into_iter() {
        ...
        for (k, v) in account.1.storage.into_iter() {
            ...
            storage_trie
                .insert(&hash_key, &alloy_rlp::encode_fixed_size(&v))
                .unwrap();
        }
...
mod tests {
    ...
    fn test_basic_state_root() {
        ...
        for (k, v) in test.into_iter() {
            ...
            trie.insert(&address_hash, &hex::decode(v).unwrap())
                .unwrap();
        }

        assert_eq!(
            hex::encode(trie.root_hash().unwrap().as_bytes()),
            "a7c787bf470808896308c215e22c7a580a0087bb6db6e8695fb4759537283a83"
        );
    }
